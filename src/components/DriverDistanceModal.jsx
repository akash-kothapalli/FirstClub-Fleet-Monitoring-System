  ...
  async function handleSelectDriver(driver) {
    ...
    try {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const foundDrv = (res.drivers || []).find(drv => drv.driver_id === driver.driver_id || drv.vehicle_id === driver.vehicle_id);
        history.push({
          date: dateString,
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                ...
                <tbody>
                  {data?.drivers?.map(drv => (
                    <tr key={drv.driver_id || drv.vehicle_id} style={{ borderBottom: '1px solid #1e293b', background: selectedDriver?.driver_id === drv.driver_id ? 'rgba(56, 189, 248, 0.1)' : 'transparent' }}>
